...
public class Buscaramigo extends javax.swing.JFrame {
    ...
    private void BTN_AMIGOSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BTN_AMIGOSActionPerformed
        ...
        String valus2 = user_txt.getText();
        var manejoarchivo = new ManejoArchivo();
        var strError = "";

        if (searchTerm.isEmpty() || valus2.isEmpty()) {
            ...
            try {
                ABB.WriteTree("C:/MEIA/lista_amigos.txt");
                ABB.actualizarDescriptor(); // Actualizar el descriptor
                clear();
            } catch (IOException ex) {
    ...
    private void jbbuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbbuscarActionPerformed
        String searchTerm = jtbuscar.getText();
        Login jj = new Login();
        String vus = jj.usuarioActual;
        if (searchTerm.equals(vus)) {
            JOptionPane.showMessageDialog(this, "NO SE ENVIARA SOLICITUD A USTED MISMO", "SOLICITUD", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
